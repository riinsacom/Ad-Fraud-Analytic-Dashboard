...
def restart_app():
    """Перезапускает приложение Streamlit"""
    try:
...
def safe_execution(func):
    """Декоратор для безопасного выполнения функций с автоматическим перезапуском при ошибках"""
    @wraps(func)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    st.warning(f"Попытка {attempt + 1} из {max_retries} не удалась. Повторная попытка через {retry_delay} секунды...")
